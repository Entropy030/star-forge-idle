...
| Contract Dimension | Specification |
| :--- | :--- |
| **Status** | `COMPLETE + HUMAN APPROVED (D32)` |
...
| **Resolved Finding** | **LEGACY UPGRADE PERSISTENCE CONFLICT RESOLVED (D32):** Human decision confirmed that Legacy upgrade families (`state.upgrades.stardust`, `state.upgrades.pulsar`, `state.upgrades.singularity`) are permanent cross-run progression and must persist across Supernova. Implemented directly in `TRIGGER_SUPERNOVA` command by preserving legacy upgrade objects (level and next-cost state). Run-local categories (`state.upgrades.stellar`, etc.) remain reset at level 0. |
...
| **Human Review Gate** | Approved by Human Review (D32). |
...
| Contract Dimension | Specification |
| :--- | :--- |
| **Status** | **COMPLETE — HUMAN APPROVED — PUBLISHED AFTER THIS TASK** |
...
| **In Scope** | - Additive state field `era1.vacuumAllocation` with safe default `'BALANCED'`.<br>- Normalization in `schema.js` (no save bump; save version 17).<br>- Engine command `SET_VACUUM_ALLOCATION`.<br>- Incorporate allocation and coherence quality feedback into `getQuantumFluctuationRate`, `getEnergyDensityRate`, and `getVacuumCoherenceRates`.<br>- 3-state discrete allocation UI in Cosmos (`#cosmos-posture-controller`).<br>- Visual field allocation styling (visually distinct from Era-II plasma postures).<br>- Core visual modulation in `canvasCore.js`.<br>- Mechanic onboarding: Chrono milestone and Codex entry revealed upon Vacuum Resonance unlock.<br>- Non-blocking objective integration (contextualized explanation on `obj_era1_complete`, never blocking earlier play).<br>- Centralized provisional calibration constants in `COSMIC_REGISTRY.eraIAllocation` (`PROPAGATION: 1.5/0.5`, `BALANCED: 1.0/1.0`, `STABILIZATION: 0.5/2.5`, `qualityFeedbackStrength: 1.0`; final calibration in P5.4). |
...
| **Testing Plan** | - **Domain / Unit:** `tests/era1_allocation.test.js` (command validation, state mutation, eligibility gating, rapid toggle parity, Codex parity).<br>- **Rate Authority:** Verify `getQuantumFluctuationRate` reflects allocation and quality feedback.<br>- **Structural Characterization:** Verify PROPAGATION favors immediate throughput; STABILIZATION favors coherence; BALANCED safely progresses unattended; all modes recoverable; rapid toggling provides no advantage across tested cadences.<br>- **Persistence:** Verify v17 save loading defaults missing allocation; verify saved allocation roundtrips.<br>- **Browser:** `tests/browser/era1_allocation.spec.js` (control rendering, ARIA attributes, keyboard navigation, safe-area geometry). |
| **Human Review Gate** | Human approval of 3-state discrete control surface (Gate 1). **HUMAN REVIEW APPROVED**: controller hidden before unlock, Balanced is neutral center/default, Propagation and Stabilization communicate throughput vs coherence priorities, live readouts make tradeoffs legible, Star Core provides visible causal feedback, Era I is distinct from Era II, mobile 390x844 passed, reduced-motion semantics remain legible. |
...
| Contract Dimension | Specification |
| :--- | :--- |
| **Status** | **COMPLETE — HUMAN APPROVED — PUBLISHED** |
...
| **In Scope** | - Resolve confirmed simulation/presentation drift identified in the Authority Matrix.<br>- Consolidate `tempMultiplier` calculation behind single authoritative helper `getTemperatureMultiplier(state)` / `recalcTempMultiplier(state)`.<br>- **AutoCompress Parity:** Connect AutoCompress to `getCompressionHeatYield(state)`, update `tempMultiplier`, `stats.maxTemp`, and stage promotion identically to manual compression.<br>- Reconcile AutoCompress execution cadence (deterministic rate $1\text{ comp/s/lvl}$ with `era3.autoCompressProgress`).<br>- Reconcile Hydrogen rate presentation (`getStellarRates` vs `getHydrogenGenRate`) to eliminate false displayed rates.<br>- Connect active declared modifiers (Gravity milestones, first-Supernova achievement, Singularity upgrades, constant $\alpha$, constant $\hbar$) where verified active.<br>- Clean up dead / unapplied legacy references (e.g. `autoSynthesize` or `fusionDiscount`). |
...
| **Files Involved** | `src/eras/stellar/authority.js`, `src/eras/stellar/simulation.js`, `src/eras/stellar/commands.js`, `src/eras/stellar/selectors.js`, `src/core/economy.js`, `src/core/stellar.js`, `src/ui/viewport.js`, `src/config/registry.js`, `src/state/createInitialState.js`, `src/state/schema.js`. |
| **State / Schema Impact** | Additive `era3.autoCompressProgress` normalized in schema (default `0`). Save version strictly 17. |
| **Testing Plan** | - Unit tests verifying manual and automated compression yield identical heat and state updates.<br>- Parity tests verifying `getStellarRates` matches actual simulation deltas.<br>- Characterization tests ensuring Era-III progression remains stable under reconciled rates.<br>- 59 fast-test files (429 tests), 36 Playwright browser specs, lint clean, build clean, git diff clean. |
| **Human Review Gate** | Human review of reconciled Stellar Authority. **HUMAN REVIEW APPROVED**: single state-explicit authority established, Hydrogen base 10 provisional, Fusion discount floor 1 H before Efficient, Alpha throughput active across He/C/Fe, AutoSynthesize velocity active across C/Fe, First Supernova +10% active, AutoCompress manual parity and deterministic cadence active, single Solar Flare authority active, Fusion Surge active, hbar active, Thermal State Index presentation decoupled from production multiplier, second-run stability deferred, save version 17 preserved. |
...
| **Files Involved** | `src/eras/stellar/simulation.js`, `src/eras/stellar/commands.js`, `src/eras/stellar/selectors.js`, `src/eras/stellar/authority.js`, `src/engine/cosmosPresentation.js`, `src/engine/resourcePresentation.js`, `src/ui/viewport.js`, `src/ui/cosmosExperience.js`, `src/ui/resourceFormatters.js`, `src/ui/canvasCore.js`, `src/content/codex.js`, `src/core/objectiveDefinitions.js`, `index.html`. |
| **State / Schema Impact** | None. Operates on existing Era III state fields (save version 17). |
...
| **Testing Plan** | - **Structural Tests:** Verify Gravity affects inflow/containment; Fusers govern throughput; actual fusion is stock-limited; Temperature scales reaction velocity; Compression increases temperature.<br>- **Bottleneck Tests:** Verify derived bottleneck shifts when investments resolve constraints; adversarial upstream sustainable rate checks.<br>- **Strategy Robustness:** Verify Gravity-heavy is not dominant; Greedy/Adaptive play remains viable; Remnant outcomes preserved.<br>- **Full-Run Gateway Tests:** Verify progression to 2B K, 1,000 Fe, and Supernova completes reliably.<br>- **Browser Suite:** Dedicated `tests/browser/era3_hydrostatic_engine.spec.js` covering desktop and mobile 390x844 geometry and visual review. |
| **Human Review Gate** | Human approval of Era-III Primary Semantic Slot (Gate 2). **HUMAN REVIEW APPROVED**: D29 hydrostatic model active, single reaction capability display truth established, rate precision explains diagnostics, mobile 390x844 geometry and scrollability verified, Forge transition settling confirmed, 41 browser tests passing. |
