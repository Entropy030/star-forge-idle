...
if (harnessStart !== -1) {
  const harnessEnd = main.indexOf('if (typeof window !== "undefined" && typeof document !== "undefined") {', harnessStart);
  if (harnessEnd !== -1) {
    main = main.substring(0, harnessStart) + main.substring(harnessEnd);
  }
}
...
main = main.replace(
  ...
  "import { gameState, setGameState, isDirty, setIsDirty, saveGame, deserializeState, loadGame, serializeState } from './core/state.js';"
);
