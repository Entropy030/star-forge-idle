...
28. **P5.4A Natural Full-Run Characterization complete, reconciled, and human approved:** Established durable characterization harness (`tests/p5_natural_run.test.js`) explicitly separating three evidence lanes:
    - **Lane A: Natural Fresh-State Observation (Bounded):** Traces natural progression starting from `createInitialState()` through Fresh Era I $\to$ Inflation $\to$ Era II $\to$ Recombination $\to$ Era III Protostar $\to$ Main Sequence Star ($10\text{M K}$) across three playstyle profiles: Informed ($2,529.9\text{s}$ total to Main Sequence), Low-Attention Balanced ($2,918.2\text{s}$), and Simple ($2,974.7\text{s}$). Full natural Supernova was **not observed** under current bounded natural profiles due to late Era III compression widening.
    - **Lane B: Exact Authority / Mathematical Projection:** Sequentially characterized the zero-meta baseline compression curve using current production authority (`executeCompression`, `getCompressionScaling`, `getCompressionHeatYield`). Confirmed exact zero-meta crossings: Main Sequence ($10\text{M K}$) at Compression #3 ($56\text{ cumulative He}$), Carbon Nucleosynthesis ($500\text{M K}$) at Compression #22 ($2,736,471\text{ cumulative He}$), and Iron Synthesis ($2.0\text{B K}$) at Compression #32 ($737,176,323\text{ cumulative He}$). Reconciled fuel authority ($10\text{ H / He}$ baseline, $\max(1, 10 - 2 \times \text{fusionDiscount}) / (1 + 0.10 \times \text{efficient})$) and Hydrogen inflow ($220\text{ H/s}$ at Gravity 20 with $+10\%$ milestone multiplier).
    - **Lane C: Prepared-State Prestige Characterization:** Validated Supernova execution and D32 Legacy upgrade persistence on a prepared Supernova-ready fixture; verified second-run acceleration ($92.2\text{M K}$ at $t = 300\text{s}$ with auto-compression).
    - **Key Findings:** **No structural defect identified in characterized scope.** **Late Era III severe calibration defect confirmed.** The compression cost/heat relationship ($1.75^n$ cost vs $1.15^n$ heat) is the primary confirmed driver of the late-era stall. Recombination OR condition reaffirmed ($1,000,000\text{ Protons}$ OR $\le 3,000\text{ K}$). Save schema Version 17 strictly preserved. Verified across 62 fast-test files (477 vitest tests), build clean, lint clean, and diff clean. **P5.4A Milestone Complete & Human Approved.** Next activity: P5.4B Calibration Modeling & Parameter Sweep.
